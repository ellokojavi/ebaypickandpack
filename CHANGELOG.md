...
- **Bulk shipping now actually applies its notes.** The batch path always called
  ...
  even counted the notes — but `NOTE_ADD_KEY` held a **single**
  `{ orderId, note }` object. Each order overwrote the slot and opened a
  *background* note tab, which Firefox throttles; order N's write landed while
  order N-1's tab was still reading, so N-1 threw "Note data mismatch or
  missing" — and that error path deliberately leaves the tab open. Any tab that
  *did* succeed then set the whole key to `null`, taking every still-pending
  sibling's note down with it. Migrated to a map keyed by order id, with
  `queueOrderNote` / `peekOrderNote` / `consumeOrderNote` mirroring the buyer
  message helpers (the same fix that shape needed in an earlier release, never
  applied here). Simulated at batch timing: 12 orders with throttled note tabs
  applied **1/12** notes before, **12/12** after. Well-separated single-card
  shipping scored 12/12 either way — which is exactly why this only ever showed
  up in bulk.
- **Every shipped order now gets a dated note, not only deferred ones.** A card
  set to Today previously got no note at all, leaving no record on eBay of when
  it actually went out. Today ships now write `Shipped on <date>`; Tomorrow ships
  keep `Will be shipped on <date>`. Both go through the existing
  `shipNoteSent` guard, so retrying a failed shipment still never double-adds.
- **Failures are reported against the right order.** The note error path read
  the order id back out of shared storage; mid-batch that was often a *different*
  order, so one failure could mark an innocent card's note as failed. It now
  reports against the order in the tab's own URL.
- Note-dialog text is inserted through `setAndTriggerInputValue` rather than a
  raw `.value =` plus hand-rolled events, so there is one way of typing into a
  field across the whole script (see v4.41).
- Batch pre-flight dialog now itemises `"Will be shipped on"` and `"Shipped on"`
  notes separately, so the counts match what the run will actually do.
