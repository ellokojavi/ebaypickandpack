...
- **Status read from eBay, not inferred** 🔎 — when the "Mark as shipped" button doesn't appear, the script reads eBay's own order **progress stepper** rather than assuming the order already shipped. Shipped → confirmed, with eBay's ship date recorded. Not shipped, or no stepper on the page → reported as a genuine failure with the tab left open for you to look at
